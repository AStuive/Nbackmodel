...
class Editor extends JEditorPane
{
	...
	Editor (final Frame frame, Preferences prefs)
	{
		...
		inputMap.put (KeyStroke.getKeyStroke (KeyEvent.VK_UP, accelerator + ActionEvent.SHIFT_MASK), new AbstractAction () {
			public void actionPerformed (ActionEvent e) {
		...
		inputMap.put (KeyStroke.getKeyStroke (KeyEvent.VK_DOWN, accelerator + ActionEvent.SHIFT_MASK), new AbstractAction () {
			public void actionPerformed (ActionEvent e) {
		...
		inputMap.put (KeyStroke.getKeyStroke (KeyEvent.VK_LEFT, 0), new AbstractAction () {
			public void actionPerformed (ActionEvent e) {
				...
				else
				{
					...
					if (pos > 0) editor.setCaretPosition (pos-1);
				}
			}
		});
		inputMap.put (KeyStroke.getKeyStroke (KeyEvent.VK_RIGHT, 0), new AbstractAction () {
			public void actionPerformed (ActionEvent e) {
				...
				{
					int pos = editor.getCaretPosition();
					if (pos < editor.getModelDocument().getLength()) editor.setCaretPosition (pos+1);
				}
			}
		});
	...
	Document getModelDocument () { return document; }
	...
	{
		try { return openHelp (new FileInputStream (file), suppressStyling); }
		catch (Exception e) { return false; }
	}

	boolean open (URL url)
	{
		try { return openHelp (url.openStream(), false); }
		catch (Exception e) { return false; }
	}
	...
	void moveCaretNextBlock (boolean select)
	{
		...
		if (select) moveCaretPosition (start);
		else setCaretPosition (start);
	...
	void addMarkers (Iterator<ParseError> pes, boolean output)
	{
		...
		while (pes.hasNext())
		{
			...
			try
			{
				...
				if (pe.getLine() == lastLine) remove (getComponentCount() - 1);
				add (marker);
				updateMarker (marker);
				if (output) frame.output (pe.getText() + " on line " + pe.getLine());
			}
