...
class Document extends DefaultStyledDocument
{
	...
	private class UndoQueue
	{
		...
		UndoQueue (int max) { this.max=max; v=new Vector<UndoState>(); }
		int size () { return v.size(); }
		void push (UndoState ui) { v.add(ui); if (v.size()>max) v.removeElementAt(0); }
		UndoState pop () { UndoState ui=v.lastElement(); v.removeElementAt(v.size()-1); return ui; }
		void clear () { v.clear(); }
		int lastOffset () { return (v.size()==0) ? 0 : v.lastElement().selectionStart; }
	}
	...
	void disableExtras () { extrasEnabled = false; }
	...
	int findNextParen (char paren, int offset)
	{
		...
		for ( ; offset < length ; offset++)
		{
			...
			if (c==')' && parenLevel>0) parenLevel--;
			if (parenLevel==0 && c==paren) return offset;
			if (c=='(') parenLevel++;
		}
		return length-1;
	}
	...
	{
		int i = 0;
		while (i<markers.size() && markers.elementAt(i).getOffset() <= offset) i++;
		if (i >= markers.size()) return getLength();
	...
	int findPreviousMarker (int offset, Vector<Marker> markers)
	{
		...
		while (i>=0 && markers.elementAt(i).getOffset() >= offset) i--;
		if (i < 0) return 0;
		return markers.elementAt(i).getOffset();
	}

	Vector<Marker> getCommandMarkers () { return commands; }
	Vector<Marker> getProductionMarkers () { return productions; }
	Vector<Marker> getChunkMarkers () { return chunks; }
	...
	public void insertString (int offset, String text, AttributeSet attr) throws BadLocationException
	{
		...
		{
			changed = true;
			if (text.length()==1) undoKeystrokes++;
			if (undos.size()==0 || Math.abs(offset-undos.lastOffset())>maxUndoKeystrokes
					|| text.length()!=1 || undoKeystrokes >= maxUndoKeystrokes)
				saveState();
		}
		super.insertString (offset, text, styleNormal);
		if (extrasEnabled) restyle();
	}
	...
	void restyle ()
	{
		if (suppressStyling) return;
		if (restyleThread != null) restyleThread.cancel (false);
		restyleThread = new SwingWorker<Object,Object>() {
			public Object doInBackground() {
				try { Thread.sleep (restyleDelay); } catch (Exception e) { }
				return null;
			}
			protected void done() {
				if (!isCancelled())
	...
	{
		try { return getText(pos,1).charAt(0); }
	...
	private void recolor () throws BadLocationException
	{
		...
		if (prefs.autoHilite) setCharacterAttributes (0, length, styleNormal, true);
		while (pos < length)
		{
			while (pos<length && isWhite(charAt(pos))) pos++;
			if (pos >= length) break;
			...
			if (c == '(')
			{
				...
				while (pos<length && isWhite(charAt(pos))) pos++;
				int start = pos;
				while (pos<length && notWhiteOrSpecial(charAt(pos))) pos++;
				String keyword = getText(start,pos-start);
				if (parenLevel == 1)
				{
					if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleCommand, false);
					commands.add (new Marker ("("+keyword+" ...)", createPosition(parenPos)));
					lastCommand = keyword;
				}
				else if (lastCommand.equals("add-dm"))
				{
					if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleChunk, false);
				...
				if (keyword.toLowerCase().equals("p"))
				{
					...
					start = pos;
					while (pos<length && notWhiteOrSpecial(charAt(pos))) pos++;
					if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleProduction, false);
			...
			else if (c == ')')
			{
				...
				if (parenLevel==0) inProductionRule = false;
			}
			else if (c == ':' && !inProductionRule)
			{
				int start = pos;
				while (pos<length && notWhiteOrSpecial(charAt(pos))) pos++;
				if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleParameter, false);
			}
			else if (c == '*')
			{
				int start = pos;
			...
			{
				int start = pos;
				while (pos+1<length && (charAt(pos)!='|' || text.charAt(pos+1)!='#')) pos++;
				if (pos<length) pos++;
				if (pos<length) pos++;
				if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleComment, false);
			}
			else if (c==';' || c=='#')
			{
				int start = pos;
			...
			else if (c=='=' || c=='+' || c=='-' || c=='?')
			{
				...
				if (buffer.endsWith(">") && buffer.length()>3)
					if (prefs.autoHilite) setCharacterAttributes (start, pos-start, styleBuffer, false);
			}
			else if (c=='!')
			{
				int start = pos;
	...
	private void reindent () throws BadLocationException
	{
		...
		while (pos < getLength())
		{
			...
			if (firstToken.startsWith("(")) realParenLevel = startParenLevel;
			else if (parenLevel<startParenLevel && !firstToken.startsWith(")")) realParenLevel = startParenLevel;
			...
			{
				if (firstToken.equals(")")) realParenLevel = 0;
				else if (firstToken.equals("==>")) realParenLevel = 0;
	...
	boolean canUndo () { return undos.size() > 0; }
	boolean canRedo () { return redos.size() > 0; }
