...
/**
 * Conditions tested for a buffer as specified in the (left-hand) condition side of an ACT-R production rule.
 *  
...
class BufferCondition
{
	 ...
	 * @param ba2 the second buffer action
	 * @return <tt>true</tt> if the two buffer conditions are the same, or <tt>false</tt> otherwise
	 */
	 ...
	 */
	public Symbol getBuffer() { return buffer; }

	/** 
	 * Gets the number of slot conditions in this buffer action. 
	 * @return the number of slot conditions
	 */
	public int slotCount() { return slotConditions.size(); }

	/** 
	 * Gets the slot condition for a given index.
	 * @param i the slot condition index
	 * @return the slot condition at the given index
	 */
	public SlotCondition getSlotCondition (int i) { return slotConditions.elementAt(i); }

	/** 
	 * Gets an iterator for all slot conditions. 
	 * @return an iterator that iterates over all slot conditions
	 */
	public Iterator<SlotCondition> getSlotConditions() { return slotConditions.iterator(); }

	/** 
	 * Gets the slot condition for a given slot.
	 * @param slot the slot
	 * @return the slot condition for that slot, or <tt>null</tt> if the slot is not present 
	 */
	 ...
	 * @param slot the slot
	 * @returns <tt>true</tt> if the buffer condition specifies the slot in any slot condition, and <tt>false</tt> otherwise 
	 */
	public boolean hasSlotCondition (Symbol slot) { return getSlotCondition(slot)!=null; }

	/** 
	 * Checks whether the buffer condition specifies a given slot value.
	 * @param value the slot value
	 * @returns <tt>true</tt> if the buffer condition specifies the slot value in any slot condition, and <tt>false</tt> otherwise 
	 */
	...
	/** 
	 * Checks whether the buffer condition is a special condition denoted by the <tt>!</tt> prefix.
	 * @return <tt>true</tt> if the buffer condition is special, or <tt>false</tt> otherwise
	 */
	public boolean isSpecial () { return (prefix == '!'); }
		...
		for (int i=0 ; i<slotConditions.size() ; i++)
		{
			...
			if (model.getProcedural().whyNotTrace)
			{
				...
				if (model.getProcedural().whyNotTrace)
				{
					...
					else model.output (slotCondition.toString());
				}
	...
	boolean test (Instantiation inst)
	{
		if (prefix == '!')
		{
			...
			for (int i=0 ; i<specials.size() ; i++)
			{
				...
				if (Symbol.get(special).isVariable()) special = inst.get(Symbol.get(special)).getString();
				tokens.add (special);
			}
			try
			{
	...
	/** 
	 * Gets a string representation of the buffer condition with an instantiation. 
	 * @return the string
	 */
	...
	/** 
	 * Gets a string representation of the buffer condition, with standard indentation. 
	 * @return the string
	 */
