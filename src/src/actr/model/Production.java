...
/**
 * Condition-action production rules that serve as the primary representation of cognitive skill.
 *  
...
public class Production
{
	 ...
	 * @return <tt>true</tt> if the two productions are the same, or <tt>false</tt> otherwise
	 */
	 ...
	 * @return <tt>true</tt> if the production has a reward, or <tt>false</tt> otherwise
	 */
	...
	void setReward (double reward) { this.reward = reward; hasReward = true; }
	void clearReward () { reward = 0; hasReward = false; }
	
	boolean isBreakPoint() { return breakPoint; }
	...
	boolean queriesOtherThanBusy (Symbol buffer)
	{
		...
		if (bc.slotCount()==0) return false;
	...
	Instantiation instantiate (Buffers buffers)
	{
		...
		if (model.getProcedural().whyNotTrace) model.output (name.getString());
		...
		for (int i=0 ; i<conditions.size() ; i++)
		{
			...
			if (! bc.test (inst))
			{
				if (model.getProcedural().whyNotTrace) model.output ("   X instantiation failed\n");
		...
		while (it.hasNext())
		{
			...
			if (model.getProcedural().whyNotTrace) model.output ("   [delayed] " + dsc.buffer + ">");
		...
		if (model.getProcedural().whyNotTrace) model.output ("   * instantiation succeeded: "+inst+"\n");
		model.getProcedural().whyNotTrace = savedWhyNotTrace;
		return inst;
	}
	...
	void fire (Instantiation inst)
	{
		...
		if (goal!=null) goal.setLastUsedAsGoal (model.getTime());
		...
		for (int i=0 ; i<conditions.size() ; i++)
		{
				...
				)
			{
				...
				if (!found) model.getBuffers().clear (bc.getBuffer());
				else model.getBuffers().touch (bc.getBuffer());
			}
		}
		for (int i=0 ; i<actions.size() ; i++)
		{
	...
	{
		if (value==null) { model.outputError ("cannot specialize "+variable+" to null value"); return; }
		for (int i=0 ; i<conditions.size() ; i++)
	...
	public String toString (Instantiation inst)
	{
		...
		String s = "(p " + name + "\n";
		for (int i=0 ; i<conditions.size() ; i++) s += conditions.elementAt(i).toString(inst,used);
		s += "==>\n";
	...
	/** 
	 * Gets a string representation of the production, with standard indentation. 
	 * @return the string
	 */
