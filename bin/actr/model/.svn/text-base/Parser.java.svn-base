...
class Parser
{
	...
	void parse (Model model, String taskOverride)
	{
		try
		{
			...
			{
				if (!t.getToken().equals("(")) model.recordError (t);
				t.advance();
				...
				if (t.getToken().equals("set-task"))
				{
					...
					if (taskOverride == null)
					{
						...
						else model.recordError ("task '" + taskName + "' is not defined", t);
					}
					t.advance();
					if (!t.getToken().equals(")")) model.recordError (t);
					t.advance();
				...
				else if (t.getToken().equals("set-parameter"))
				{
					...
					while (t.hasMoreTokens() && !t.getToken().equals(")"))
					{
						...
						t.advance();
						if (t.getToken().equals(")")) { model.recordError (t); break; }
				...
				else if (t.getToken().equals("goal-focus"))
				{
					...
					if (c!=null) model.getBuffers().set (Symbol.goal, c);
					else model.recordWarning ("chunk '" + goalName + "' has not been defined", t);
					if (!t.getToken().equals(")")) model.recordError (t);
					t.advance();
				}
				else if (t.getToken().equals("spp"))
				{
					...
					if (p == null)
					{
						...
						while (t.hasMoreTokens() && !t.getToken().equals(")")) t.advance();
						t.advance();
						break;
					}
					while (t.hasMoreTokens() && !t.getToken().equals(")"))
				...
				else if (t.getToken().equals("set-base-levels"))
				{
					...
					while (t.hasMoreTokens() && !t.getToken().equals(")"))
					{
						...
						else model.recordWarning ("chunk '" + c + "' has not been defined", t);
						if (!t.getToken().equals(")")) model.recordError (t);
						t.advance();
					}
					if (!t.getToken().equals(")")) model.recordError (t);
	...
	Production parseProduction (Model model) throws Exception
	{
		...
		//t.advance();
		if (!t.getToken().equals("p") && !t.getToken().equals("p*")) model.recordError (t);
		t.advance();
	...
	BufferCondition parseBufferCondition (Model model, Set<String> variables) throws Exception
	{
		...
		if (prefix=='=') variables.add ("=" + bufferName);
	...
	SlotCondition parseSlotCondition (Model model, Set<String> variables) throws Exception
	{
		...
		if (slot.isVariable()) variables.add (slot.getString());
		t.advance();
		Symbol value = Symbol.get (t.getToken());
		if (value.isVariable()) variables.add (value.getString());
		t.advance();
	...
	Chunk parseChunk (Model model) throws Exception
	{
		...
		else model.recordWarning ("redefining chunk '" + name + "'", t);
