...
public class U4Paired extends Task
{
	...
	public void update (double time)
	{
		...
		if (pairIndex >= pairs.length)
		{
			...
			iteration++;
			if (iteration >= runIterations) getModel().stop();
			currentTrial = new Trial();
			pairIndex = 0;
			pairItem = 0;
		}
	...
	public Result analyze (Task[] tasks, boolean output)
	{
		...
		{
			getModel().output ("\n=====\n");
			getModel().output ("Latency [R = " +
					String.format("%.2f",Statistics.correlation (modelTimes, humanTimes)) + "]:");
		...
		result.add ("U4Paired RT", modelTimes, humanTimes);
		result.add ("U4Paired Corr", modelCorrect, humanCorrect);
		return result;
	}
